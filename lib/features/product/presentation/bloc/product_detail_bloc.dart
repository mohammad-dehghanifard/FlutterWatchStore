...
class ProductDetailBloc extends Bloc<ProductDetailEvent, ProductDetailState> {
  ...
  ProductDetailBloc(this.useCase) : super(ProductDetailState(detailStatus: ProductDetailLoading())) {
    ...
    on<FetchProductDetailEvent>((event, emit) async {
      ...
      if(response == null || response.result == false){
        emit(state.copyWith(ProductDetailError(response?.message ?? "خطایی رخ داده لطفا با پشتیبانی تماس بگیرید")));
      } else {
        emit(state.copyWith(ProductDetailSuccess(response)));
      }
    });
  }
}
